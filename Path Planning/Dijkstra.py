...
if __name__ == '__main__':
	...
	# Calculate Dijkstra path and plot in a subfigure
	plt_dijkstra = plt.subplot(1,2,1)
	# Provide size and color to all the nodes in graph G_dijkstra
	nx.draw(G_dijkstra, pos, node_size = 40, node_color = 'w', with_labels = False)
	nx.draw_networkx_edges(G_dijkstra, pos, width=1.0, alpha=0.5)
	# Run Dijkstra and provide color blue and size 100 to all nodes in the shortest path
	path_d = dijkstra_path(G_dijkstra, source, destination)
	title_dijkstra = 'Dijkstra - Route length ' + str(len(path_d))
	plt.title(title_dijkstra)
	# Color the route with Blue
	for idx in range(len(path_d)):
		nx.draw_networkx_nodes(G_dijkstra, pos, nodelist=[path_d[idx]], node_color='b', node_size = 100)
		if idx != len(path_d)-1:
			nx.draw_networkx_edges(G_dijkstra, pos, edgelist=[(path_d[idx], path_d[idx+1])], edge_color='b', width = 3)

	#Overwrite the color as red-green and size of source-estination nodes
	nx.draw_networkx_nodes(G_dijkstra, pos, nodelist=[source, destination], node_color=['r','g'], node_size = 200)
	plt.axis('image')
